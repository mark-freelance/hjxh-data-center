...
import com.mulaobao.administration.dao.TmPxbDao;
 ...
 */
@Service("tmPxbService")
public class TmPxbServiceImpl extends ServiceImpl<TmPxbDao, TmPxb> implements TmPxbService {
    ...
    public void getInformation(MultipartFile multipartFile, String ms, LsShop shop_name,Date data1) {
        ...
        try {
            ...
            EasyExcel.read(multipartFile.getInputStream(), TmPxb.class,new TmPxbListener(tmPxbService,stringObjectHashMap)).sheet().doRead();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void getInformation1(String file, String ms, LsShop shop_name, Date date2) {
        ...
        stringObjectHashMap.put("DateTime",date2);
            EasyExcel.read(new File(file), TmPxb.class,new TmPxbListener(tmPxbService,stringObjectHashMap)).sheet().doRead();
    }

    @Override
    public Map<String, Object> getNum(String shopBh, Date data1) {
        QueryWrapper<TmPxb> ew = new QueryWrapper<>();
        ew.eq("Shop_bh", shopBh );
        ew.eq("Date_time",data1);
        ew.select("SUM(Consume) as Pinxiaobaofy ,SUM(Hits_num) as Pinxiaobaoll ");
        Map<String, Object> map = tmPxbService.getMap(ew);
        return map;
    }
}
